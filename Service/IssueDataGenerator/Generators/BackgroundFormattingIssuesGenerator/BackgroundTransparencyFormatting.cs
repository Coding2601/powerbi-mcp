...
namespace PowerBI_MCP.Service.IssueDataGenerator.Generators.BackgroundFormattingIssuesGenerator
{
    public class BackgroundTransparencyFormatting : IIssueDataGenerator
    {
       public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.BackgroundFormatting["Transparency"]))
                    {
                        inconsistentVis[vis.BackgroundFormatting["Transparency"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.BackgroundFormatting["Transparency"], [vis]);
                    }
                }
                catch (Exception ex)
                {
