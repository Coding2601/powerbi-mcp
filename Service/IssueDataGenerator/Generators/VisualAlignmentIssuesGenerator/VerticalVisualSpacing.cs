...
{
    public class VerticalVisualSpacing : IVisualAlignmentIssueGenerator
    {
                            ...
                            try
                            {
                                ...
                                foreach (var otherVisual in visuals)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            if (!(otherX + otherWidth <= x || otherX >= x + width))
                                            {
                                                double topDistance = y - (otherY + otherHeight);
                                                if (nearestVisuals["top"].distance == -1 || topDistance < nearestVisuals["top"].distance)
                                                {
                                                    nearestVisuals["top"] = (topDistance, otherVisual.VisualId ?? "");
                                                }
                                            }
                                        }
                                        ...
                                        if (otherY >= y + height)
                                        {
                                            if (!(otherX + otherWidth <= x || otherX >= x + width))
                                            {
                                                double bottomDistance = otherY - (y + height);
                                                if (nearestVisuals["bottom"].distance == -1 || bottomDistance < nearestVisuals["bottom"].distance)
                                                {
                                                    nearestVisuals["bottom"] = (bottomDistance, otherVisual.VisualId ?? "");
                                                }
                                            }
                                        }
                                    }
