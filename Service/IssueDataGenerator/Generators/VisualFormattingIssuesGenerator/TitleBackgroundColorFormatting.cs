...
{
    public class TitleBackgroundColorFormatting : IIssueDataGenerator
    {
          public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    string? bgColor = vis.WithoutTitleFormatting["BackgroundColor"]?.ToString()?.Trim('\'')?.ToLower();
                    if (inconsistentVis.ContainsKey(bgColor))
                    {
                        inconsistentVis[bgColor].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(bgColor, [vis]);
                    }
                }
                catch (Exception ex)
                {
