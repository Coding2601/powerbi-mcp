...
{
    public class TitleFontSizeFormatting : IIssueDataGenerator
    {
        public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.WithoutTitleFormatting["FontSize"]))
                    {
                        inconsistentVis[vis.WithoutTitleFormatting["FontSize"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.WithoutTitleFormatting["FontSize"], [vis]);
                    }
                }
                catch (Exception ex)
                {
