...
{
    public class TitleAlignmentFormatting : IIssueDataGenerator
    {
          public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.WithoutTitleFormatting["Alignment"]))
                    {
                        inconsistentVis[vis.WithoutTitleFormatting["Alignment"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.WithoutTitleFormatting["Alignment"], [vis]);
                    }
                }
                catch (Exception ex)
                {
