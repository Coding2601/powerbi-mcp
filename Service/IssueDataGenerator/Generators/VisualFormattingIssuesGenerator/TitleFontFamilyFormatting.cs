...
namespace PowerBI_MCP.Service.IssueDataGenerator.Generators.VisualFormattingIssuesGenerator
{
    public class TitleFontFamilyFormatting : IIssueDataGenerator
    {
          public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.WithoutTitleFormatting["FontFamily"]))
                    {
                        inconsistentVis[vis.WithoutTitleFormatting["FontFamily"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.WithoutTitleFormatting["FontFamily"], [vis]);
                    }
                }
                catch (Exception ex)
                {
