...
namespace PowerBI_MCP.Service.IssueDataGenerator.Generators.DataLabelFormattingIssuesGenerator
{
    public class DataLabelFontFamilyFormatting : IIssueDataGenerator
    {
          public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.DataLabelFormatting["FontFamily"]))
                    {
                        inconsistentVis[vis.DataLabelFormatting["FontFamily"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.DataLabelFormatting["FontFamily"], [vis]);
                    }
                }
                catch (Exception ex)
                {
