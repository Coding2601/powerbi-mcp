...
{
    public class DataLabelOrientationFormatting : IIssueDataGenerator
    {
          public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.DataLabelFormatting["Orientation"]))
                    {
                        inconsistentVis[vis.DataLabelFormatting["Orientation"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.DataLabelFormatting["Orientation"], [vis]);
                    }
                }
                catch (Exception ex)
                {
