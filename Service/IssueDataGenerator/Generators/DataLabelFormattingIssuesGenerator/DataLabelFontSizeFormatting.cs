...
{
    public class DataLabelFontSizeFormatting : IIssueDataGenerator
    {
          public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.DataLabelFormatting["FontSize"]))
                    {
                        inconsistentVis[vis.DataLabelFormatting["FontSize"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.DataLabelFormatting["FontSize"], [vis]);
                    }
                }
                catch (Exception ex)
                {
