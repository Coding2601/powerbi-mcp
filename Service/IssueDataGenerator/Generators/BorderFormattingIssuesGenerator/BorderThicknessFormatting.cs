...
{
    public class BorderThicknessFormatting : IIssueDataGenerator
    {
        public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.BorderFormatting["Thickness"]))
                    {
                        inconsistentVis[vis.BorderFormatting["Thickness"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.BorderFormatting["Thickness"], [vis]);
                    }
                }
                catch (Exception ex)
                {
