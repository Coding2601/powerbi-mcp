...
{
    public class BorderRadiusFormatting : IIssueDataGenerator
    {
        public SingleIssueRuleData GetData(IssueContext issueContext)
        {
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    if (inconsistentVis.ContainsKey(vis.BorderFormatting["Radius"]))
                    {
                        inconsistentVis[vis.BorderFormatting["Radius"]].Add(vis);
                    }
                    else
                    {
                        inconsistentVis.Add(vis.BorderFormatting["Radius"], [vis]);
                    }
                }
                catch (Exception ex)
                {
