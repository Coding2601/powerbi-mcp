...
{
    public class BackgroundContrast : IIssueDataGenerator
    {
        ...
        private List<object> GetColorNotValidBackgroundContrastData(IssueContext issueContext)
        {
            // visual background color from theme json
            ...
            foreach (var vis in issueContext.ReportDocumentation?.VisualList ?? [])
            {
                try
                {
                    Dictionary<string, List<Dictionary<string, string>>> visualColors = vis.ExtractedColors;
                    List<string> colorList = [];
                    string? backgroundColor =
                        vis.BackgroundFormatting["Visibility"] != "false" && vis.BackgroundFormatting["Color"] != "Default" && vis.BackgroundFormatting["Color"] != null
                            ? vis.BackgroundFormatting["Color"].Trim().Replace("'", "")
                            : null;
                    if (backgroundColor != null)
                    {
                        JObject visualStyles = (JObject)(theme?["visualStyles"]?[vis.OgVisualType ?? "*"]?["*"] ?? asteriskVisualStyles);
                        backgroundColor =
                            visualStyles?["background"]?.FirstOrDefault()?["solid"]?["color"]?["expr"]?["ThemeDataColor"] != null
                                ? GlobalHandler.GetThemeDataColorToHex(
                                    (JObject)(visualStyles["background"]?.FirstOrDefault()?["solid"]?["color"]?["expr"]?["ThemeDataColor"] ?? new JObject()),
                                    theme?["dataColors"]?.ToObject<List<string>>() ?? []
                                )
                                : (visualStyles?["background"]?.FirstOrDefault()?["solid"]?["color"]?.ToString()?.Trim()?.Replace("'", "") ?? "#FFFFFF");
                    }
                    string finalBgColor = backgroundColor ?? "#FFFFFF";
                    ...
                    foreach (var pair in foreAndBackgroundPairColors)
                    {
                        try
                        {
                            ...
                            bool isDistinguishable1 = ColorContrastHandler.IsForegroundColorAccessibleFromBackground(pair.ForegroundColor, pair.BackgroundColor);
                            if (isDistinguishable1 == false)
                            {

                                string? error = $@"Color {CreateColorIndicatorBlock(pair.ForegroundColor)}  {pair.ItemParentCategory + " " + pair.ItemLocation}  have not proper contrast with background color {CreateColorIndicatorBlock(pair.BackgroundColor)}";
                                if (error != null)
                                {
                                    ...
                                    errorDict["visualTitle"] = vis.VisualTitle ?? ""; errorDict.Add("error", error);
        ...
        {
            List<object> issueData = GetColorNotValidBackgroundContrastData(issueContext) ?? [];
